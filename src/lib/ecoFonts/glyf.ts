...
import { SCALE, ecoHoleContours, subtractEcoHoles } from "./geometry";
import { cubicToQuadratics, fitContour } from "./curvefit";
import type { Pt } from "./curvefit";

type GlyfPoint = { x: number; y: number; on: boolean };

const YIELD_EVERY = 24;
/** Bézier refit tolerance (fraction of em). Tighter than upstream ecofont.ts
 * to reduce silhouette wobble after hole subtraction. */
const FIT_TOLERANCE_EM = 0.0008;
...
export async function ecoProcessTrueType(
    ...
    const segLen = upem / 120; // finer flatten → less outline distortion after Clipper
    ...
    for (let gid = 0; gid < numGlyphs; gid++) {
        ...
        if (raw.length > 0) {
            ...
            const original = readGlyphBounds(view, start);
            const flat = readGlyphContours(view, bytes, glyf.offset, offsets, gid, segLen, 0);
            if (flat && flat.length > 0) {
                // Prefer appending hole contours so the original exterior (and
                // its curves) stay byte-identical — avoids silhouette warp on
                // large display glyphs like page-number "4".
                if (numContours >= 0) {
                    const holes = ecoHoleContours(flat, upem, intensity);
                    const native = readSimpleGlyphPoints(view, bytes, start, numContours);
                    if (holes && holes.length > 0 && native && native.length > 0) {
                        const withHoles = appendHoleContours(native, holes);
                        const serialized = writeGlyfPoints(withHoles, original);
                        if (serialized) {
                ...
                if (!replacement) {
                    const holed = subtractEcoHoles(flat, upem, intensity);
...
function readSimpleGlyphPoints(
    view: DataView,
    bytes: Uint8Array,
    start: number,
    numContours: number,
...
function contourArea(pts: GlyfPoint[]): number {
    let a = 0;
...
function outerContourSign(native: GlyfPoint[][]): number {
    // Contour 0 is not always the exterior (counters may be listed first).
    let bestArea = 0;
    for (const c of native) {
        const a = contourArea(c);
        if (Math.abs(a) > Math.abs(bestArea)) bestArea = a;
    }
    return Math.sign(bestArea) || 1;
}

function appendHoleContours(native: GlyfPoint[][], holes: ClipPaths): GlyfPoint[][] {
    const outerSign = outerContourSign(native);
    const extras: GlyfPoint[][] = [];
    for (const hole of holes) {
        if (hole.length < 3) continue;
        let pts: GlyfPoint[] = hole.map((p) => ({
        ...
        // Eco holes must wind opposite the exterior so TrueType nonzero fill cuts.
        if (Math.sign(contourArea(pts)) === outerSign) pts.reverse();
        extras.push(pts);
    }
    return extras.length > 0 ? native.concat(extras) : native;
}

function writeGlyfPoints(
    contours: GlyfPoint[][],
    original: Bounds,
): { bytes: Uint8Array; pointCount: number; contourCount: number } | null {
    if (contours.length === 0) return null;
    let pointCount = 0;
    for (const c of contours) pointCount += c.length;
    ...
    w.uint16(0); // drop hints — point indices would be wrong after appended holes
