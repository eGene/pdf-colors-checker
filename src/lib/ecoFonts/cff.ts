...
import { SCALE, subtractEcoHoles } from "./geometry";
import { ecoProcessTrueType, findSfntTable, replaceSfntTable } from "./glyf";
import { fitContour, type Pt } from "./curvefit";

const YIELD_EVERY = 24;
/** Match glyf surgeon — curve refit after Clipper flatten. */
...
/** Process FontFile3 payload: OpenType/TrueType sfnt or bare CFF. */
export async function ecoProcessFontFile3(
    bytes: Uint8Array,
    intensity: number,
    onGlyph?: (done: number, total: number) => void,
): Promise<ArrayBuffer> {
    ...
    if (version === 0x4f54544f || version === 0x00010000 || version === 0x74727565) {
        return ecoProcessSfnt(bytes, intensity, onGlyph);
    }
    return (await ecoProcessCff(bytes, intensity, onGlyph)).buffer;
}
...
/**
 * Emit a Type 2 charstring from Clipper polygons. Refits Béziers so outer
 * silhouettes stay closer to the pre-hole curves (pure rlineto chains warped
 * large display digits in judge evals).
 */
function buildCharstring(polys: ClipPaths, widthRaw: number | null, upem: number): Uint8Array {
    const out: number[] = [];
    ...
    for (const poly of polys) {
        ...
        const pts: Pt[] = poly.map((pt) => ({ x: pt.X / SCALE, y: pt.Y / SCALE }));
        const { start, segs } = fitContour(pts, tolerance);
        const sx = Math.round(start.x);
        const sy = Math.round(start.y);
        encodeCsNumber(out, sx - cx);
        encodeCsNumber(out, sy - cy);
        out.push(21); // rmoveto
        cx = sx;
        cy = sy;
        for (const seg of segs) {
            if (seg.type === "line") {
                const ex = Math.round(seg.end.x);
                const ey = Math.round(seg.end.y);
                encodeCsNumber(out, ex - cx);
                encodeCsNumber(out, ey - cy);
                out.push(5); // rlineto
                cx = ex;
                cy = ey;
            } else {
                const c1x = Math.round(seg.c1.x);
                const c1y = Math.round(seg.c1.y);
                const c2x = Math.round(seg.c2.x);
                const c2y = Math.round(seg.c2.y);
                const ex = Math.round(seg.end.x);
                const ey = Math.round(seg.end.y);
                encodeCsNumber(out, c1x - cx);
                encodeCsNumber(out, c1y - cy);
                encodeCsNumber(out, c2x - c1x);
                encodeCsNumber(out, c2y - c1y);
                encodeCsNumber(out, ex - c2x);
                encodeCsNumber(out, ey - c2y);
        ...
        try {
            ...
            if (parsed.contours.length > 0) {
                ...
                if (holed && holed.length > 0) {
                    newCharstrings[gid] = buildCharstring(holed, parsed.widthRaw, upem);
                    glyphsChanged++;
                }
            }
        } catch {
